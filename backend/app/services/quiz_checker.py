...
"""
    ...
    try:
        ...
        raise ValueError(
            "IMac-AI returned invalid quiz JSON."
        ) from exc
...
async def generate_answer_key(image_bytes: bytes, mime_type: str = "image/jpeg"):
    ...
    prompt = """
You are IMac-AI's Quiz Answer Key Generator.
...
"""
    ...
    try:
        ...
        raise ValueError(
            "IMac-AI returned invalid answer-key JSON."
        ) from exc
    ...
    if not isinstance(questions, dict):
        raise ValueError(
            "Invalid answer-key format returned by IMac-AI."
        )
...
async def check_student_answers(
    ...
    prompt = f"""
You are IMac-AI's Student Quiz Grader.

The answer key has already been generated by IMac-AI.
...
"""
    ...
    try:
        ...
        raise ValueError(
            "IMac-AI returned invalid grading JSON."
        ) from exc
    ...
    if not isinstance(questions, dict):
        raise ValueError(
            "Invalid grading format returned by IMac-AI."
        )
